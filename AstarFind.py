...
def getOrder(temp,operation,delta,m,zeroPos):
    ...
    for i in range(delta):
        if i%2 == 0:
            ...
            operation.append(m)
    if delta%2:
        temp[zeroPos],temp[changeId[zeroPos][change[reverse[m]]]] = temp[changeId[zeroPos][change[reverse[m]]]],temp[zeroPos]
...
def bfsHash(start, zeroPos,des,step,change_position):
    ...
    while not que.empty():
        ...
        if len(tempN.operation) == step and tempN.flag == 0:#符合强制交换条件，开始执行变换操作
            ...
            temp[change_position[0]-1],temp[change_position[1]-1] = temp[change_position[1]-1],temp[change_position[0]-1]
            swap = []
...
def getProblemImageOrder(stuid):
    ...
    single_weight = weight / 3
    for i in range(3):
        for j in range(3):
            dst = img[int(i * single_height):int((i + 1) * single_height),
                  int(j * single_weight):int((j + 1) * single_weight)]
            # img_list.append(dst)
            # 在本目录生成被分割的图片
